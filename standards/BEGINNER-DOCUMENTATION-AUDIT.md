...
|---:|---|---|---|---|---|
| 1 | B1-1 | 필수 | ✅ 존재 / 메인 직접 연결 | 🟡 정합성 감사 필요 | **P0 — 현재 미션** |
| 2 | B1-2 | 필수 | ✅ 존재 / 메인 직접 연결 | 🟡 정합성 감사 필요 | P1 |
| 3 | B2-1 | 필수 | ✅ 존재 / 메인 직접 연결 | 🟡 정합성 감사 필요 | P1 |
| 4 | B2-2 | 필수 | ✅ 존재 / 메인 직접 연결 | 🟡 정합성 감사 필요 | P1 |
| 5 | B3-1 | 필수 | ✅ 존재 / 메인 직접 연결 | 🟡 정합성 감사 필요 | P1 |
| 6 | B3-2 | 필수 | ✅ 존재 / 메인 직접 연결 | 🟡 정합성 감사 필요 | P1 |
| 7 | B4-1 | 필수 | ✅ 존재 / 메인 직접 연결 | 🟡 정합성 감사 필요 | P1 |
| 8 | B5-1 | 필수 | ✅ 존재 / 메인 직접 연결 | 🟡 정합성 감사 필요 | P1 |
| 9 | B6-1 | 필수 | ✅ 존재 / 메인 직접 연결 | 🟡 정합성 감사 필요 | P1 |
| 10 | B6-2 | 필수 | ✅ 존재 / 메인 직접 연결 | 🟡 정합성 감사 필요 | P1 |
| 11 | B7-1 | 필수 Term Project | ✅ 존재 / 메인 직접 연결 | 🟡 정합성 감사 필요 | P1 |
| 12 | B4-2 | 선택 | ✅ 존재 / 메인 직접 연결 | 🟡 정합성 감사 필요 | P2 |
| 13 | B5-2 | 선택 | ✅ 존재 / 메인 직접 연결 | 🟡 정합성 감사 필요 | P2 |
| 14 | B5-3 | 선택 | ✅ 존재 / 메인 직접 연결 | 🟡 정합성 감사 필요 | P2 |
| 15 | B7-2 | 선택 Term Project | ✅ 존재 / 메인 직접 연결 | 🟡 정합성 감사 필요 | P2 |

`🟡 정합성 감사 필요`는 가이드가 없다는 뜻이 아닙니다. **가이드는 이미 존재하지만 과거 Phase/Runtime/OS 표현, 현재 실행 경로, 새 문서 생성 표준 적용 여부를 다시 확인해야 한다는 뜻**입니다.

## 3. 알려진 Documentation Drift 유형

기존 가이드에는 다음과 같은 과거 상태 표현이 남아 있을 수 있습니다.
