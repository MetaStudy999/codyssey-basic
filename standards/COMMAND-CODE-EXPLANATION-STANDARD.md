# 명령어·코드 한 줄 해설 표준(Command & Code Explanation Standard)

Codyssey Basic의 입문자 문서는 학습자가 명령어나 코드를 단순 복사하는 데서 끝나지 않고, **실행 가능한 명령 한 줄과 의미 있는 코드 한 줄이 무엇을 하는지 자기 말로 설명할 수 있도록 작성**합니다.

이 표준은 `BEGINNER-GUIDE.md`, 환경 따라하기, 문제 해결(Troubleshooting), 평가 가이드(Evaluation Guide) 등 입문자가 실제로 명령·코드·SQL을 실행하는 문서에 적용합니다.
...
- 비밀정보(Secret), Token, Password, Private Key 값을 출력하는 명령은 예제로 사용하지 않습니다.
- 실제 값을 채팅·README·증빙(Evidence)에 복사하도록 유도하지 않습니다.
...
예상 출력은 실제 증빙(Evidence)과 구분합니다. 실제 실행하지 않은 예시를 PASS/CLEAR로 기록하지 않습니다.
...
## 10. 복사·붙여넣기 안전(Copy & Paste Safety)
...
복사·붙여넣기(Copy/Paste): 실행본과 해설/출력 분리
출력(Output): 정상 범위와 변동 가능 값 구분
재실행(Rerun): 상태 변경 명령은 필요한 경우 안전 등급 표시
```
...
[ ] 위험 명령과 비밀정보(Secret)에 대한 안전 기준을 지킨다.
```
