# Terminology Standard — 한글·영어 병기 표준
...
문맥이 충분히 형성된 뒤에는 한글 또는 영어 원어 하나만 사용할 수 있습니다.
...
| 검증(Verify) | 요구사항이 실제로 충족됐는지 확인 |
| 증빙 자료(Evidence) | 실행·검증 결과를 확인할 수 있는 기록 |
| 완료(CLEAR) | 요구·검증·증빙까지 충족된 상태 |
| 실행 전 점검(Preflight) | Runtime 전에 위험·누락을 확인하는 단계 |
...
기존 15개 미션의 모든 문장을 한 번에 대량 수정하는 것은 피합니다. 현재 Runtime을 막지 않는 기존 문서는 보존하고, **사용자가 실제로 읽는 입문자 경로와 새로 수정하는 문서부터 이 표준을 적용**합니다.

```text
새 문서/현재 수정 문서
→ Terminology Standard 적용

기존 문서
→ Runtime blocker/학습 혼동이 있으면 우선 교정
