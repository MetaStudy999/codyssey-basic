...
R01 실제 실행 순서
B1-1 → B1-2 → ... → B7-1 → B4-2 → B5-2 → B5-3 → B7-2

중요
실행 순서 ≠ 모든 미션 사이의 필수 의존성
```

현재 어떤 미션을 실행해야 하는지는 [NEXT-ACTIONS.md](NEXT-ACTIONS.md), 전체 순서는 [`../../MISSION-INDEX.md`](../../MISSION-INDEX.md)를 확인합니다.

## 📑 목차

- [1. 필수 선행 미션](#required)
- [2. 권장 선행 미션](#recommended)
- [3. 있으면 좋은 선행 지식](#prior-knowledge)
- [4. B7-1에 수렴하는 학습 전이](#b7-1-transfer)
- [5. B7-2 고도화 경로](#b7-2-path)
...
flowchart LR
    ...
    B53[B5-3 FastAPI Auth] -. 인증/관계 보강 .-> B71
```
...
선행 관계가 실제 학습에 사용되도록, 관련 미션 저장소의 `training/round-01-clear/START-CHECK.md`에서 **미션 완료 여부가 아니라 현재 보유 지식**을 먼저 확인합니다.
...
- `START-CHECK.md`의 체크 문항은 **공식 시험이나 합격 점수표가 아닙니다.**
- 필수 선행이 없는 미션은 권장 선행 미션을 CLEAR하지 않았더라도 필요한 개념을 알고 있으면 바로 시작할 수 있습니다.
- 일부 개념이 부족하면 해당 권장 미션 전체를 다시 하는 대신 필요한 Step/개념만 먼저 보충할 수 있습니다.
- B7-2는 예외적으로 B7-1 결과물을 실제 기반으로 요구하므로 B7-1 준비 여부를 필수로 확인합니다.
