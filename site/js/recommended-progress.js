(() => {
  ...
  function estimatedCellProgress(cell, telemetry) {
    ...
    if (missionTelemetry?.gates) {
      ...
      if (gates.length) {
        return Math.round((gates.reduce((sum, value) => sum + gateScore(value), 0) / gates.length) * 100);
      }
    }
  ...
  function missionChips(cells) {
    ...
    return cells.map(cell => `
      <span class="recommended-mission-chip" title="${escapeHtml(cell.title || cell.title_en || cell.mission)}">
        ${escapeHtml(cell.mission)}
      </span>
    `).join('');
  }
  ...
  function render() {
    ...
    const complete = cells.filter(cell => statusMap[cell.mission] === 'COMPLETE');
    const active = cells.filter(cell => activeStatuses.has(statusMap[cell.mission]));

    const dependenciesSatisfied = cell => (dependencies[cell.mission] || [])
      .every(dep => statusMap[dep] === 'COMPLETE');

    const ready = cells.filter(cell => {
      const status = statusMap[cell.mission];
      if (status === 'COMPLETE' || activeStatuses.has(status)) return false;
    ...
    const nextUnlock = waiting.filter(cell => {
      const unmet = (dependencies[cell.mission] || []).filter(dep => statusMap[dep] !== 'COMPLETE');
    ...
    if (complete.length === cells.length && cells.length > 0) {
      recommendation = '✅ 모든 Workcell이 COMPLETE입니다. 남은 공식 Serial Integration과 증거 정합성을 확인하는 단계가 권장됩니다.';
    } else if (active.length) {
      recommendation = `🛠 ${ids(active)} 마무리를 우선하고`;
      if (nextUnlock.length) recommendation += ` → 🔓 ${ids(nextUnlock)} 해제를 확인하세요.`;
      else recommendation += '.';
      if (ready.length) recommendation += ` 동시에 🎯 ${ids(ready)}는 병렬 진행할 수 있습니다.`;
    } else if (ready.length) {
      recommendation = `🎯 현재 ${ids(ready)}를 병렬 진행하는 것이 권장됩니다.`;
    } else {
      recommendation = '⏳ 현재 후행 미션은 선행 Workcell COMPLETE 확인이 필요합니다. 수동 Live 상태를 갱신해 최신 상태를 확인하세요.';
    }
    ...
    root.innerHTML = `
      ...
      <div class="recommended-progress-bars">
        <div class="recommended-meter">
          <div class="recommended-meter-meta">
            <span>📊 Live 추정 진행률</span>
            <strong>${averageLive}% · COMPLETE ${complete.length}/${cells.length}</strong>
          </div>
          <div class="recommended-meter-track" role="progressbar" aria-label="Live estimated Workcell progress" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${averageLive}">
            <span class="recommended-meter-fill" style="width:${averageLive}%"></span>
          </div>
        </div>
        <div class="recommended-meter">
          <div class="recommended-meter-meta">
            <span>🏛 Official Serial Integration</span>
            <strong>${integratedPercent}% · ${integrated}/${cells.length}</strong>
          </div>
          <div class="recommended-meter-track" role="progressbar" aria-label="Official serial integration progress" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${integratedPercent}">
            <span class="recommended-meter-fill recommended-meter-fill-official" style="width:${integratedPercent}%"></span>
          </div>
        </div>
      </div>

      <div class="recommended-flow" aria-label="Current recommended Workcell flow">
        <div class="recommended-step recommended-step-complete">
          <div class="recommended-step-title"><span class="recommended-icon">✅</span><span>완료</span></div>
          <div class="recommended-mission-list">${missionChips(complete)}</div>
        </div>
        <div class="recommended-arrow" aria-hidden="true">→</div>
        <div class="recommended-step recommended-step-active">
          <div class="recommended-step-title"><span class="recommended-icon">🛠</span><span>진행 중 · 마무리 우선</span></div>
          <div class="recommended-mission-list">${missionChips(active)}</div>
        </div>
        <div class="recommended-arrow" aria-hidden="true">→</div>
        <div class="recommended-step recommended-step-ready">
          <div class="recommended-step-title"><span class="recommended-icon">🎯</span><span>지금 병렬 진행 권장</span></div>
          <div class="recommended-mission-list">${missionChips(ready)}</div>
        </div>
        <div class="recommended-arrow" aria-hidden="true">→</div>
        <div class="recommended-step recommended-step-waiting">
          <div class="recommended-step-title"><span class="recommended-icon">⏳</span><span>선행 완료 대기</span></div>
          <div class="recommended-mission-list">${missionChips(waiting)}</div>
          ${nextUnlock.length ? `<div class="recommended-unlock">🔓 다음 해제 예상: ${escapeHtml(ids(nextUnlock))}</div>` : ''}
        </div>
      </div>

      <p class="recommended-callout"><strong>권장:</strong> ${escapeHtml(recommendation)}</p>
      <p class="recommended-note">※ 이 흐름은 현재 Workcell 상태와 Control Tower의 운영상·학습상 권장 dependency를 함께 사용합니다. 공식 prerequisite 또는 PASS를 새로 판정하지 않으며, 아래 수동 갱신 버튼으로 Live telemetry를 갱신하면 이 권장 흐름도 함께 다시 계산됩니다.</p>
    `;
  }
  ...
  if (workcellGrid) {
    ...
    observer.observe(workcellGrid, {childList: true, subtree: true});
  }
